...
def already_part_of_df(chain, wallet_address):
    ...
    df = pd.read_csv('grain_user_data.csv')
...
# # gets the user_data for a given user
def get_user_data(contract, wallet_address, wait_time):

    user_data = contract.functions.getUserData(wallet_address).call()

    time.sleep(wait_time)

    return user_data

# # gets the vesting length a user chose
def get_user_terms_length(user_data):
    terms_length = user_data[0]

    return terms_length

# # gets how much grain a user is owed
def get_user_total_owed(user_data):
    
    total_owed = user_data[1]

    total_owed = total_owed / 1e18

    return total_owed

# # gets how much pending grain a user is owed
def get_user_pending_rewards(user_data):

    pending_rewards = user_data[2]

    pending_rewards = pending_rewards / 1e18

    return pending_rewards

# # gets how much grain a user has claimed
def get_user_total_claimed(user_data):

    total_claimed = user_data[3]

    total_claimed = total_claimed / 1e18

    return total_claimed

# # gets how much a grain a user can actively claim
def get_user_grain_to_claim(user_data):

    grain_left_to_claim = user_data[4]

    grain_left_to_claim = grain_left_to_claim / 1e18
...
def make_lge_data_csv(df):
    old_df = pd.read_csv('grain_user_data.csv')
    old_df = old_df.drop_duplicates(subset=['wallet_address','grain_owed','chain'], keep='last')

    combined_df_list = [df, old_df]
    combined_df = pd.concat(combined_df_list)
    combined_df = combined_df.drop_duplicates(subset=['wallet_address','grain_owed','chain'], keep='last')

    if len(combined_df) >= len(old_df):
        combined_df.to_csv('grain_user_data.csv', index=False)
...
def loop_through_rpc(df, ui_data_provider_contract, wait_time, grain_to_claim):
    ...
    for wallet_address in wallet_address_list:
        ...
        if exists == False:
            ...
            user_data = get_user_data(ui_data_provider_contract, wallet_address, wait_time)

            user_terms_length = get_user_terms_length(user_data)
            
            user_grain_owed = get_user_total_owed(user_data)

            user_pending_grain = get_user_pending_rewards(user_data)

            total_user_grain_claimed = get_user_total_claimed(user_data)
...
def find_single_rpc_vests(lge_csv, chain, ui_data_provider_contract_address, wait_time, total_grain_to_claim, web3):
...
def find_chains_vested_grain(chain_index, lge_csv):
    ...
    for rpc_url in rpc_url_list:
        ...
        grain_found += find_single_rpc_vests(lge_csv, chain_symbol, ui_data_provider_contract_address, wait_time, total_grain_to_claim, web3)
